...
function onEnableVibration(event) {
	window.localStorage.setItem('enable-vibration', !!document.getElementById('enable-vibration').checked);
}
...
function openSettings() {
	...
	document.getElementById('enable-vibration').checked = "true" === window.localStorage.getItem('enable-vibration');
...
function startResetProgressHold() {
	stopResetProgressHold();
	document.querySelector('.reset-container').classList.add('reset-progress-animation');
	window._resetProgressTimeout = setTimeout(resetProgressCallback, 4000);
}

function stopResetProgressHold() {
	document.querySelector('.reset-container').classList.remove('reset-progress-animation');
