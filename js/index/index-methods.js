(() => {
  ...
  window.upgradeCell = (previousCellElement, cellElement) => {
		...
		chainData.list.forEach((eachDisplayName) => {
			...
			if (Array.isArray(eachDisplayName)) {
				...
				eachDisplayName.forEach((eachInListInList) => {
					...
					listInList.push(
						`<div title="${eachInListInList}"${getSelectedCellBorderStyle(displayNameInCell,
							eachInListInList
			...
			collectEach.push(
				`<div title="${eachDisplayName}"${getSelectedCellBorderStyle(displayNameInCell,
					eachDisplayName
				)}><img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${
					pokemonData.id
				}.png" /></div>`
	...
	window.decorateBackpackCell = (cellElement, pokemonDisplayName, evolutionCount) => {
	...
	window.onPokeBallClick = () => {
		...
		if (
			...
			const cellElement = document.getElementById(`shuffled-cell-${indexWithoutDisplayName}`);
	
			randomizeBackpackCell(
				cellElement,
				Number.parseInt(cellElement.getAttribute('data-evolution-count'), 10) - 1
			);
